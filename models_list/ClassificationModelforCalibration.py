...
class CloneModelforCalibration(nn.Module):
    def __init__(self, encoder, config, tokenizer, args):
        super(CloneModelforCalibration, self).__init__()
...
class DefectModelforCalibration(nn.Module):
    def __init__(self, encoder, config, tokenizer, args):
        super(DefectModelforCalibration, self).__init__()
        self.encoder = encoder
        self.config = config
        self.tokenizer = tokenizer
    ...
    def forward(self, source_ids=None, labels=None):#source_ids.shape=batch[0].shape=batch_size*max_len512
