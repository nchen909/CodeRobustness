...
def main():
    ...
    if args.sample_rate==-1:
        ...
        args.few_shot = int(args.sample_rate * get_sample_size(args,type='train'))
    ...
    #                                           target_ids=target_ids, target_mask=target_mask)
                        ...
                        outputs = model(input_ids=source_ids, attention_mask=source_mask,
                                        labels=target_ids, decoder_attention_mask=target_mask)
                        # print("*********tflops:**********")
                        # # input_dict={'input_ids':source_ids,'attention_mask':source_mask,
                        # #                 'labels':target_ids,'decoder_attention_mask':target_mask}
                        # print("models.__class__:",model.__class__)
                        # if args.prefix_tuning:
                        #     print("tflops:",get_tflops(model,batch))
                        # else:
                        #     from thop import profile
                        #     # dict_={'input_ids':source_ids,'labels':target_ids}
                        #     macs, params = profile(model, inputs=(batch, ))
                        #     print(macs)
                        #     # print("tflops:",get_tflops(model,(source_ids.detach().clone(), target_ids.detach().clone(), source_mask.detach().clone(), target_mask.detach().clone())))
                        #     # input_dict = {'input_ids': source_ids, 'labels': target_ids}
            ...
            for cur_epoch in range(args.start_epoch, int(args.num_train_epochs)):
                ...
                for step, batch in enumerate(bar):
                    if args.sub_task == "POJ":
                        ...
                        # print("*********tflops:**********")
                        # print(get_tflops(model,(source_ids,p_inputs,n_inputs,labels)))
                        # print("**************************")
                    else:
                        if args.model_name == 'unixcoder':
                            source_ids = batch[0].to(args.device)        
                            ...
                            # print("*********tflops:**********")
                            # print(get_tflops(model,(source_ids, labels)))
                            # print("**************************")
